...
The system SHALL provide `rules infer --rules <file> --source-world <world> --source-coordinate <x,y,z> --target-world <world> --target-coordinate <x,y,z>`, SHALL load the supplied rule file as read-only inference context, and SHALL accept exactly one source observation and one target observation, with the source and target argument groups defining the transformation direction. The command SHALL accept independent `--source-dimension <dimension>` and `--target-dimension <dimension>` arguments, each defaulting to `overworld`.

#### Scenario: Accept a complete pair and establish direction from arguments
- **WHEN** readable, compatible worlds and valid coordinates are supplied through the source and target argument groups
- **THEN** the command treats the former as source evidence and the latter as target evidence, loads both observations, and attempts to produce one rule document
